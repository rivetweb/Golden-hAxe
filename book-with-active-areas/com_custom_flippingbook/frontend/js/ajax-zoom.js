...
function fb_ajaxZoom(page_number) {
	...
	var image_url=flippingBook.enlargedImages[page_number];
	if (image_url.substr(image_url.length - 1) == "|") 
		image_url = image_url.substr(0, image_url.length - 1);
		
	var swf_width=flippingBook.swfWidth[page_number]; 
	var swf_height=flippingBook.swfHeight[page_number];
